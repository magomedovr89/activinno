...
#     await message.answer('Пока, пока, давай иди отсюда')


# @dp.message_handler(commands=['summator', '+'])
# async def summator(message: types.Message):
#     try:
#         await message.reply(sum(map(float, message.text.split()[1:])))
#     except:
#         await message.reply('Дяденька за базаром следи')
